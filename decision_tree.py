...
########################################################################################################
    ...
    #print(labels)
    train_features, test_features, train_labels, test_labels = train_test_split(features, labels, test_size=0.33, random_state=1)#划分训练集和测试集
    clf = DecisionTreeClassifier(criterion='gini')#调用生成决策树分类器
    clf = clf.fit(train_features, train_labels)
    dot_data = tree.export_graphviz(clf, out_file=None,#可视化决策树
