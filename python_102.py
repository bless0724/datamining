...
def  psfeatureTime(data):#计算特征
    ...
    featuretime_list = [df_mean, df_std, df_rms, df_skew, df_kurt, df_boxing, df_fengzhi, df_maichong, df_yudu, df_qiaodu]
...
########################################################################################################
    s1_feature = []
    s1_list = list_of_groups(s1_filter1, 3000) #每一秒为一帧
    count = int(len(s1_filter1) / 3000)  # 计算有多少帧
    for i in range(0, count): #每一秒计算特征值
        s1_feature.append(psfeatureTime(np.array(s1_list[i])))
###################################################################################################################

#画表
###################################################################################################################
    idx = pd.Index(np.arange(1, count+1)) #行索引
    df_feature = pd.DataFrame(s1_feature, index=idx, columns=['均值', '标准差', '均方根', '偏度', '峰度','波形因子','峰值因子','脉冲因子','裕度因子','峭度'])#dataframe格式保存，行为每一秒，列为特征值
    vals = np.around(df_feature.values, 5) #数值保留5位小数
    fig = plt.figure()
    plt.rcParams['font.sans-serif']=['SimHei']
    plt.rcParams['axes.unicode_minus'] = False #设置可以显示中文
    ax = fig.add_subplot(111, frameon=True, xticks=[], yticks=[])
    the_table = plt.table(cellText=vals, rowLabels=df_feature.index, colLabels=df_feature.columns, #画表
